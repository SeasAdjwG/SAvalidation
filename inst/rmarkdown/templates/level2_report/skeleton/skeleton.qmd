...
if(comb_test_res=="None"){
  message_to_print <- kableExtra::kbl(cbind("Combined test:",
                                            comb_test_res),
                                      col.names = NULL, table.attr = 'data-quarto-disable-processing="true"',escape = FALSE) |>
    row_spec(1,background = "#FFCDCD")
}else{
  message_to_print <- kable(cbind("Combined test:",
                                  comb_test_res),
...
if(check_negatives(sa)){
  ...
  tab_to_print <- dplyr::tibble(Date=zoo::as.Date(stats::time(sa)),
                                SA=sa)|>
    dplyr::filter(sa<0)|>
    dplyr::mutate(SA = cell_spec(SA,background = "#FFCDCD"))
  kableExtra::kbl(digits = 3, format.args = list(digits = 3, nsmall = 3), align = "lrr", table.attr = 'data-quarto-disable-processing="true"', escape = FALSE) |> 
    kableExtra::kable_styling(full_width = FALSE, bootstrap_options = c("basic", "striped"), font_size = 12)
}else{
  message_to_print <- 
