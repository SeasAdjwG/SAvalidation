...
if(default_type=="TS"){
  ...
  nsa_mod5 <- RJDemetra::jtramoseats(window(nsa,start=start_5yr),
                                    spec = default_spec_nsa)
  sa_mod5 <- RJDemetra::jtramoseats(window(sa,start=start_5yr),
                                   spec = default_spec_sa)
}
if(default_type=="X13"){
  ...
  sa_mod <- RJDemetra::jx13(sa,
                           spec = default_spec_sa)
  nsa_mod5 <- RJDemetra::jx13(window(nsa,start=start_5yr),
                             spec = default_spec_nsa)
  sa_mod5 <- RJDemetra::jx13(window(sa,start=start_5yr),
                            spec = default_spec_sa)
...
if(check_for_calendar_vars(sa_mod)){
  ...
  cal_effect <- RJDemetra::get_indicators(sa_mod, "preprocessing.model.cal")[[1]]
}else{
  message_to_print <- 
...
residuals_tests_names <- sprintf("diagnostics.%s",
                                  c("seas-lin-f",
                                    ...
                                    "seas-lin-friedman",
                                    "seas-lin-periodogram"))
test_results <- do.call(rbind,RJDemetra::get_indicators(sa_mod5, residuals_tests_names))
dplyr::tibble(Test=c("F-test","QS","Kruskall-Wallis","Friedman","Periodogram"),
              `Test statistic` = test_results[,1],
              `P-value` = test_results[,2]) |>
  dplyr::mutate(`P-value` = cell_spec(sprintf("%.3f", `P-value`),
